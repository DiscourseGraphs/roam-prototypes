/* A source-level guard for a bug two sibling prototypes shipped.
 *
 ...
 * load. Named imports are unaffected. Vitest cannot catch this: it resolves
 * CommonJS with ordinary interop, so the same code passes every unit test and
 * only fails in the built bundle. Hence a check on the source text.
 */
...
describe("module interop", () => {
  ...
  it("never uses the legacy top-level roamAlphaAPI aliases", () => {
    const offenders = sourceFiles(SRC).flatMap((file) =>
      readFileSync(file, "utf8")
        .split("\n")
        .filter((line) => /roamAlphaAPI\.(q|pull|createBlock|updateBlock|createPage)\b/.test(line))
        .map((line) => `${file.replace(SRC, "src")}: ${line.trim()}`),
    );
    expect(offenders).toEqual([]);
  });
